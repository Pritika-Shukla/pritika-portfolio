...
export const FloatingDock = ({
  ...
  onItemClick?: (item: { title: string; icon: React.ReactNode; href: string; onClick?: () => void }) => void;
}) => {
  return (
    <>
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-full   px-8 pb-3 md:flex bg-[#171717]",
        className,
      )}
    >
      {items.map((item) => (
...
function IconContainer({
  ...
  return (
    <button
      onClick={() => {
        if (onItemClick) {
          onItemClick({ title, icon, href, onClick: undefined });
        }
      }}
    >
      <motion.div
